...
| **S5** | Landing runtime, durable edge→core ingestion, consent evidence, UTM + click-ID, `AnonymousTouchpoint`, `IdentityLink` | growth-web · growth-core · **auth** · bazos · leads | M1 | 🔨 **W1 (приём) развёрнут** — `POST /ingest/events` принимает и буферизует в проде. Дальше: W6 (привязка `PublisherWorker` к RabbitMQ), затем W3 → W4 → W2 → W5 по [EP-005](../21_execution_plans/EP-005-landing-and-ingestion.md) |
