...
| 6 | **Реальный вызов API** | — | ⚠️ **не выполнен** — нет refresh token | ⬜ |
...
| 11d | **Refresh token** | запустить `get-refresh-token.py` | Проверить `verify-api-access.py` | ⬜ |
...
- [MISSING: refresh token] — блокирует пункт 6 (реальный вызов API). Решается запуском `growth/scripts/get-refresh-token.py`
